...
panel.innerHTML=`
  ...
  <div class="tt31-tabs">
    ...
    <button data-tt31="table">▦ Таблиця</button>
...
Команда 4</textarea>
  ...
  <section class="tt31-section hidden" data-tt31-section="test">
    ...
    <div class="tt31-actions">
      ...
      <button id="tt33InsertTest" disabled>📄 Вставити тест на сторінку</button>
  ...
  <section class="tt31-section hidden" data-tt31-section="table">
    <div class="tt36-table-grid">
      <label>Рядки <input id="tt36TableRows" type="number" min="1" max="20" value="4"></label>
      <label>Стовпці <input id="tt36TableCols" type="number" min="1" max="12" value="4"></label>
    </div>
    <label class="tt36-check"><input id="tt36TableHeader" type="checkbox" checked> Перший рядок — заголовок</label>
    <label>Ширина <input id="tt36TableWidth" type="range" min="320" max="1000" value="680"></label>
    <div class="tt31-actions"><button id="tt36InsertTable" class="tt31-primary">▦ Вставити таблицю на дошку</button></div>
    <div class="tt31-muted">Таблицю можна рухати й масштабувати як один об’єкт.</div>
  </section>
...
if(aiCompose && !q("tt31AiTools")){
  ...
  <button type="button" data-open-tool="table">▦ Таблиця</button>
...
function renderTest(){
  ...
  q("tt33InsertTest").disabled = !testData.length;
...
function tt33TestToText(includeAnswers=false){
  if(!testData.length)return "";
  const letters=["А","Б","В","Г","Д","Е"];
  let out="ТЕСТУВАННЯ\n";
  const topic=q("tt31TestTopic")?.value?.trim();
  if(topic)out+=`Тема: ${topic}\n`;
  out+="\n";
  testData.forEach((item,idx)=>{
    out+=`${idx+1}. ${item.q}\n`;
    item.options.forEach((opt,oi)=>{
      out+=`   ${letters[oi]||String(oi+1)}. ${opt}\n`;
    });
    if(includeAnswers) out+=`   Правильна відповідь: ${letters[item.correct]||item.correct+1}\n`;
    out+="\n";
  });
  return out.trim();
}

function tt33InsertTestToBoard(){
  const text=tt33TestToText(false);
  if(!text){alert("Спочатку створіть тест.");return;}
  const ok=addTextToBoard(text,{left:250,top:150,width:700,fontSize:20});
  if(!ok) alert("Не вдалося вставити тест на сторінку.");
...
q("tt33InsertTest").onclick=tt33InsertTestToBoard;
q("tt31ClearTest").onclick=()=>{testData=[];q("tt31TestArea").innerHTML="";q("tt31TestScore").textContent="";q("tt33InsertTest").disabled=true};
...
/* Table v36 */
q("tt36InsertTable").onclick=()=>{
  const c=getCanvas();
  if(!c || !window.fabric){alert("Полотно ще не готове.");return}
  const rows=Math.max(1,Math.min(20,Number(q("tt36TableRows").value)||4));
  const cols=Math.max(1,Math.min(12,Number(q("tt36TableCols").value)||4));
  const width=Math.max(320,Number(q("tt36TableWidth").value)||680);
  const header=q("tt36TableHeader").checked;
  const cw=width/cols,ch=58,parts=[];
  for(let r=0;r<rows;r++)for(let col=0;col<cols;col++){
    parts.push(new fabric.Rect({
      left:col*cw,top:r*ch,width:cw,height:ch,
      fill:(header&&r===0)?"#eef4fb":"#ffffff",
      stroke:"#17315f",strokeWidth:1
    }));
    parts.push(new fabric.IText((header&&r===0)?`Заголовок ${col+1}`:"",
      {left:col*cw+8,top:r*ch+17,fontSize:16,fill:"#17315f",fontFamily:"Arial"}));
  }
  const g=new fabric.Group(parts,{left:220,top:170,selectable:true,evented:true});
...
/* ==================== v34 TOOLBAR CUSTOMIZER ==================== */
(function(){
  const STORAGE="sofiaToolbarLayoutV34";
  const $id=(id)=>document.getElementById(id);
  const top=document.querySelector(".top-actions");
  if(!top || $id("toolbarSettingsBtn")) return;

  const settingsBtn=document.createElement("button");
  settingsBtn.type="button";
  settingsBtn.id="toolbarSettingsBtn";
  settingsBtn.className="top-btn";
  settingsBtn.textContent="⚙ Панель";
  settingsBtn.title="Налаштувати порядок і видимість кнопок";
  const ai=$id("aiBtn");
  if(ai) top.insertBefore(settingsBtn,ai); else top.appendChild(settingsBtn);

  const panel=document.createElement("div");
  panel.id="toolbarSettingsPanel";
  panel.className="tb34-panel hidden";
  panel.innerHTML=`
    <div class="tb34-head">
      <div><b>⚙ Налаштування панелі</b><small>Перетягуйте кнопки мишкою та приховуйте непотрібні</small></div>
      <button type="button" class="tb34-close" id="tb34Close">×</button>
    </div>
    <div class="tb34-tabs">
      <button type="button" class="active" data-tb34-tab="top">Верхня панель</button>
      <button type="button" data-tb34-tab="tools">Інструменти</button>
      <button type="button" data-tb34-tab="left">Ліва панель</button>
    </div>

    <div class="tb35-settings-grid">
      <label>Кількість рядків
        <select id="tb35Rows">
          <option value="auto">Автоматично</option>
          <option value="1">1 рядок</option>
          <option value="2">2 рядки</option>
          <option value="3">3 рядки</option>
        </select>
      </label>
      <label>Розмір кнопок
        <select id="tb35Density">
          <option value="normal">Звичайні</option>
          <option value="compact">Компактні</option>
          <option value="dense">Дуже компактні</option>
        </select>
      </label>
      <label>Підписи кнопок
        <select id="tb35Labels">
          <option value="full">Значок + текст</option>
          <option value="icons">Лише значки</option>
        </select>
      </label>
      <label>Застосувати до
        <select id="tb35ApplyTo">
          <option value="top">Верхня панель</option>
          <option value="tools">Інструменти</option>
          <option value="both">Обидві панелі</option>
        </select>
      </label>
    </div>
    <div class="tb34-note">☰ Потягніть елемент угору або вниз. Зміни зберігаються на цьому пристрої автоматично.</div>
    <div id="tb34List" class="tb34-list"></div>
    <div class="tb34-actions">
      <button type="button" id="tb34ShowAll">Показати всі</button>
      <button type="button" id="tb34Reset">↺ Стандартний порядок</button>
      <button type="button" id="tb34Done" class="primary">Готово</button>
    </div>`;
  document.body.appendChild(panel);
  ...
  const groups={
    top:{
      ...
      excluded:new Set(["toolbarSettingsBtn"]),
      title:"Верхня панель"
    },
    tools:{
      container:()=>document.querySelector(".ribbon"),
      selector:".ribbon-group > button",
      excluded:new Set(["finishPolylineBtn"]),
      title:"Інструменти"
    },
    left:{
      container:()=>document.querySelector(".leftbar"),
      selector:":scope > button.side-tool",
      excluded:new Set(),
      title:"Ліва панель"
    }
  };
  ...
  function layoutState(){
    const s=getState();
    return s.layout || {
      top:{rows:"auto",density:"normal",labels:"full"},
      tools:{rows:"auto",density:"normal",labels:"full"}
    };
  }
  function normalizeLayout(l){
    l=l||{};
    l.top=l.top||{rows:"auto",density:"normal",labels:"full"};
    l.tools=l.tools||{rows:"auto",density:"normal",labels:"full"};
    return l;
  }
  function applyLayoutToRoot(root, prefs){
    if(!root)return;
    root.classList.remove("tb35-nowrap","tb35-two-rows","tb35-three-rows");
    if(prefs.rows==="1")root.classList.add("tb35-nowrap");
    if(prefs.rows==="2")root.classList.add("tb35-two-rows");
    if(prefs.rows==="3")root.classList.add("tb35-three-rows");
  }
  function applyLayoutPrefs(){
    const s=getState();
    const layout=normalizeLayout(s.layout);
    applyLayoutToRoot(groups.top.container(),layout.top);
    applyLayoutToRoot(groups.tools.container(),layout.tools);

    // Density is global on body; use the denser of selected panel settings.
    const d=[layout.top.density,layout.tools.density];
    document.body.classList.remove("tb35-compact","tb35-dense");
    if(d.includes("dense"))document.body.classList.add("tb35-dense");
    else if(d.includes("compact"))document.body.classList.add("tb35-compact");

    document.body.classList.toggle(
      "tb35-icons-only",
      layout.top.labels==="icons" || layout.tools.labels==="icons"
    );
  }
  function selectedTargets(){
    const v=$id("tb35ApplyTo")?.value||"top";
    return v==="both"?["top","tools"]:[v];
  }
  function syncLayoutControls(){
    const s=getState(),layout=normalizeLayout(s.layout);
    const target=($id("tb35ApplyTo")?.value==="tools")?"tools":"top";
    const p=layout[target];
    if($id("tb35Rows"))$id("tb35Rows").value=p.rows||"auto";
    if($id("tb35Density"))$id("tb35Density").value=p.density||"normal";
    if($id("tb35Labels"))$id("tb35Labels").value=p.labels||"full";
  }
  function saveLayoutControl(field,value){
    const s=getState();
    s.layout=normalizeLayout(s.layout);
    selectedTargets().forEach(t=>{s.layout[t][field]=value});
    setState(s);
    applyLayoutPrefs();
  }
  ...
  function cleanLabel(el){
    return (el.innerText||el.textContent||el.id||"Кнопка").replace(/\s+/g," ").trim();
  }
  function collect(groupName){
    const g=groups[groupName], root=g.container();
    if(!root)return[];
    return [...root.querySelectorAll(g.selector)].filter(el=>{
      const k=keyFor(el); return k && !g.excluded.has(el.id);
    });
  }
  function getState(){
    try{return JSON.parse(localStorage.getItem(STORAGE)||"{}")}catch(e){return{}}
  }
  function setState(s){localStorage.setItem(STORAGE,JSON.stringify(s))}
  function snapshotDefaults(){
    const s=getState();
    if(!s.defaults){
      s.defaults={};
      Object.keys(groups).forEach(name=>{
        s.defaults[name]=collect(name).map(keyFor);
      });
      setState(s);
    }
  }
  ...
  function applyState(){
    ...
    Object.keys(groups).forEach(name=>{
      const g=groups[name], root=g.container(); if(!root)return;
      const items=collect(name);
      const map=new Map(items.map(el=>[keyFor(el),el]));
      const order=state[name]?.order || [];
      const hidden=new Set(state[name]?.hidden || []);

      if(name==="tools"){
        // preserve ribbon group structure, but reorder across existing button slots
        const slots=[...root.querySelectorAll(".ribbon-group > button")].filter(el=>!g.excluded.has(el.id));
        const orderedKeys=[...order.filter(k=>map.has(k)), ...items.map(keyFor).filter(k=>!order.includes(k))];
        orderedKeys.forEach((k,i)=>{
          const el=map.get(k), slot=slots[i];
          if(el && slot && el!==slot) slot.parentNode.insertBefore(el,slot);
        });
      }else{
        const orderedKeys=[...order.filter(k=>map.has(k)), ...items.map(keyFor).filter(k=>!order.includes(k))];
        orderedKeys.forEach(k=>{const el=map.get(k);if(el)root.appendChild(el)});
        // Keep settings button at the end of top section before AI if possible? It is excluded and remains visible.
        if(name==="top" && settingsBtn.parentNode===root) root.appendChild(settingsBtn);
      }
      collect(name).forEach(el=>{
        const k=keyFor(el);
        el.classList.toggle("tb34-user-hidden",hidden.has(k));
        if(hidden.has(k)) el.style.display="none";
  ...
  function saveCurrentOrder(groupName){
    const state=getState();
    const keys=[...document.querySelectorAll("#tb34List .tb34-item")].map(x=>x.dataset.key);
    const hidden=[...document.querySelectorAll("#tb34List .tb34-item input[type=checkbox]")]
      .filter(x=>!x.checked).map(x=>x.closest(".tb34-item").dataset.key);
    state[groupName]={order:keys,hidden};
    setState(state);
    applyState();
  }

  function renderList(){
    const list=$id("tb34List"); list.innerHTML="";
    const state=getState();
    const items=collect(current);
    const map=new Map(items.map(el=>[keyFor(el),el]));
    const saved=state[current]?.order||[];
    const keys=[...saved.filter(k=>map.has(k)), ...items.map(keyFor).filter(k=>!saved.includes(k))];
    const hidden=new Set(state[current]?.hidden||[]);

    keys.forEach(k=>{
      const el=map.get(k); if(!el)return;
      const row=document.createElement("div");
      row.className="tb34-item";row.draggable=true;row.dataset.key=k;
      row.innerHTML=`<div class="tb34-handle">☰</div>
        <div class="tb34-label">${cleanLabel(el)}<small>${k.replace("#","")}</small></div>
        <label class="tb34-switch"><input type="checkbox" ${hidden.has(k)?"":"checked"}> показувати</label>`;
      list.appendChild(row);

      const cb=row.querySelector("input");
      cb.addEventListener("change",()=>saveCurrentOrder(current));

      row.addEventListener("dragstart",()=>row.classList.add("dragging"));
      row.addEventListener("dragend",()=>{row.classList.remove("dragging");saveCurrentOrder(current)});
    });

    list.addEventListener("dragover",(e)=>{
      e.preventDefault();
      const dragging=list.querySelector(".dragging"); if(!dragging)return;
      const after=[...list.querySelectorAll(".tb34-item:not(.dragging)")].find(el=>{
  ...
  settingsBtn.onclick=()=>{snapshotDefaults();panel.classList.toggle("hidden");if(!panel.classList.contains("hidden")){syncLayoutControls();renderList()}};
  $id("tb34Close").onclick=()=>panel.classList.add("hidden");
  $id("tb34Done").onclick=()=>panel.classList.add("hidden");

  panel.querySelectorAll("[data-tb34-tab]").forEach(b=>b.onclick=()=>{
    panel.querySelectorAll("[data-tb34-tab]").forEach(x=>x.classList.toggle("active",x===b));
    current=b.dataset.tb34Tab; renderList();
  });


  $id("tb35ApplyTo").addEventListener("change",syncLayoutControls);
  $id("tb35Rows").addEventListener("change",e=>saveLayoutControl("rows",e.target.value));
  $id("tb35Density").addEventListener("change",e=>saveLayoutControl("density",e.target.value));
  $id("tb35Labels").addEventListener("change",e=>saveLayoutControl("labels",e.target.value));

  $id("tb34ShowAll").onclick=()=>{
    const state=getState();
    state[current]=state[current]||{};
    state[current].hidden=[];
    setState(state);applyState();renderList();
  };

  $id("tb34Reset").onclick=()=>{
    const state=getState();
    const defaults=state.defaults?.[current]||[];
    state[current]={order:defaults,hidden:[]};
    state.layout=normalizeLayout(state.layout);
    if(current==="top" || current==="tools"){
      state.layout[current]={rows:"auto",density:"normal",labels:"full"};
    }
    setState(state);applyState();syncLayoutControls();renderList();
  };

  snapshotDefaults();
  applyState();

  // If other add-ons add buttons later, re-apply saved visibility/order gently.
