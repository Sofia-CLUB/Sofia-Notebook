(() => {
  ...
  document.documentElement.dataset.controlsVersion = "32";
  if ($("appVersionBadge")) $("appVersionBadge").textContent = "v32";

  /* ---------- FULL SCREEN v30: controls remain visible ---------- */
  let exitButton = $("v32FullscreenExit");
  if (!exitButton) {
    exitButton = document.createElement("button");
    exitButton.id = "v32FullscreenExit";
  ...
  async function enterBoardFullscreen() {
    document.body.classList.add("board-fullscreen-v32");
    try {
  ...
  async function exitBoardFullscreen() {
    document.body.classList.remove("board-fullscreen-v32");
    try {
  ...
  if (fullscreenBtn) {
    fullscreenBtn.addEventListener("click", e => {
      ...
      if (document.body.classList.contains("board-fullscreen-v32")) exitBoardFullscreen();
  ...
  document.addEventListener("fullscreenchange", () => {
    if (!document.fullscreenElement && document.body.classList.contains("board-fullscreen-v32")) {
      document.body.classList.remove("board-fullscreen-v32");
    }
  });
  ...
  let floatingDelete = $("v32ObjectDelete");
  if (!floatingDelete) {
    floatingDelete = document.createElement("button");
    floatingDelete.id = "v32ObjectDelete";
  ...
  const learnBtn = $("learningGeneratorBtn");
  ...
  /* ---------- v32: Колесо фортуни ---------- */
  const wheelBtn = $("fortuneWheelBtn");
    ...
    diagnosticButton.addEventListener("click", () => {
      setTimeout(() => {
        ...
        const checks = [
          ...
          ["Картки / тест", !!$("learningGeneratorBtn") && !!$("learningGeneratorPanel")],
          ["Колесо фортуни", !!$("fortuneWheelBtn") && !!$("fortuneWheelPanel")]
        ];
