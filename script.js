...
/* =========================================================
   V112 — ПРАВА ПАНЕЛЬ: ВИДИМА У ЗВИЧАЙНОМУ РЕЖИМІ + ЗГОРТАННЯ
          + КНОПКА "КОНТЕКСТ" ДЛЯ ТЕКСТУ/ОБ'ЄКТІВ
   ========================================================= */
(function(){
"use strict";
...
function css112(){
  ...
  st.id="v112Css";
  st.textContent=`
    /* Права панель видима і в звичайному, і в fullscreen */
    #v86Dock{
      display:flex!important;
      visibility:visible!important;
      opacity:1!important;
    ...
    /* Наш стан згортання працює в ОБОХ режимах */
    body.v112-right-collapsed #v86Dock{
      transform:translateX(100%)!important;
    }

    /* Старий V111 більше не блокує згортання */
    body.v112-right-collapsed.v86-fullscreen #v86Dock,
    body.v112-right-collapsed.v56-fullscreen #v86Dock,
    body.v112-right-collapsed.fullscreen-mode #v86Dock,
    :fullscreen body.v112-right-collapsed #v86Dock{
      transform:translateX(100%)!important;
    }
    ...
    body:not(.v112-right-collapsed) #v112RightToggle{
      right:74px!important;
    }

    /* Кнопка Контекст на правій панелі */
    #v112ContextBtn{
      width:100%!important;
      ...
      font:600 9px/1.05 Arial,sans-serif!important;
    }
    #v112ContextBtn .ico{
      font-size:18px!important;
      line-height:1!important;
    }
    #v112ContextBtn.active{
      background:#173b78!important;
      color:#fff!important;
    }

    /* Усі контекстні панелі відкриваються справа поверх усього */
    #textFormatBar.v112-context-open,
    #v102ObjectPanel.v112-context-open,
    #v68ToolSettings.v112-context-open{
      display:block!important;
      visibility:visible!important;
      opacity:1!important;
      position:fixed!important;
      ...
      left:auto!important;
      top:170px!important;
      z-index:123500!important;
      max-height:calc(100vh - 190px)!important;
      overflow:auto!important;
      background:rgba(255,255,255,.97)!important;
      border:1px solid #d8e2ef!important;
      border-radius:12px!important;
      box-shadow:0 10px 32px rgba(15,23,42,.20)!important;
      pointer-events:auto!important;
    }

    /* Горизонтальний textFormatBar робимо компактним вертикальним блоком */
    #textFormatBar.v112-context-open{
    ...
    #v112ContextEmpty.show{display:block!important}

    /* Коли права панель згорнута — контекст теж ховаємо */
    body.v112-right-collapsed #textFormatBar.v112-context-open,
    body.v112-right-collapsed #v102ObjectPanel.v112-context-open,
    body.v112-right-collapsed #v68ToolSettings.v112-context-open,
    body.v112-right-collapsed #v112ContextEmpty.show{
      display:none!important;
    }
  `;
  document.head.appendChild(st);
...
function ensureToggle112(){
  let b=$112("v112RightToggle");
  if(b) return b;

  b=document.createElement("button");
  b.id="v112RightToggle";
  b.type="button";
  ...
  b.onclick=e=>{
    ...
    const collapsed=document.body.classList.toggle("v112-right-collapsed");
    b.textContent=collapsed?"‹":"›";

    if(collapsed) closeContext112();

    try{
      localStorage.setItem("sofiaRightPanelCollapsed",collapsed?"1":"0");
    }catch(_){}

    setTimeout(resizeCanvas112,240);
  };

  return b;
}

function restoreRightState112(){
  let collapsed=false;
  try{collapsed=localStorage.getItem("sofiaRightPanelCollapsed")==="1"}catch(_){}
  document.body.classList.toggle("v112-right-collapsed",collapsed);
  const b=ensureToggle112();
  if(b)b.textContent=collapsed?"‹":"›";
}

function ensureContextButton112(){
  const dock=dock112();
  if(!dock) return;

  let b=$112("v112ContextBtn");
  if(b) return;

  b=document.createElement("button");
  b.id="v112ContextBtn";
  b.type="button";
  b.title="Відкрити контекстне меню тексту або об'єкта";
  b.innerHTML='<span class="ico">⚙</span><span>Контекст</span>';

  const help=[...dock.querySelectorAll("button")].find(x=>/Довідка/i.test((x.textContent||"").trim()));
  if(help) dock.insertBefore(b,help);
  else dock.appendChild(b);
...
function contextEmpty112(){
  let e=$112("v112ContextEmpty");
  if(e)return e;
  e=document.createElement("div");
  e.id="v112ContextEmpty";
  e.innerHTML="<b>Контекстне меню</b><br>Виділіть текст або об’єкт на аркуші, потім натисніть «Контекст».";
  document.body.appendChild(e);
  return e;
}

function closeContext112(){
  ["textFormatBar","v102ObjectPanel","v68ToolSettings"].forEach(id=>{
    const p=$112(id);
    if(!p)return;
    p.classList.remove("v112-context-open");
  });
  $112("v112ContextEmpty")?.classList.remove("show");
  $112("v112ContextBtn")?.classList.remove("active");
}

function openContext112(){
  const c=c112();
  const o=c?.getActiveObject?.();

  if(!o){
    contextEmpty112().classList.add("show");
    return;
  }

  if(isText112(o)){
    try{ if(typeof syncTextFormatBar==="function") syncTextFormatBar(); }catch(_){}
    const p=$112("textFormatBar");
    if(p){
      p.classList.remove("hidden");
      p.classList.add("v112-context-open");
      return;
    }
  }

  /* Для будь-якого не-текстового об'єкта спочатку використовуємо
     універсальну панель властивостей V102, якщо вона є. */
  const objPanel=$112("v102ObjectPanel");
  if(objPanel){
    try{
      if(typeof refreshObjectPanel==="function") refreshObjectPanel();
    }catch(_){}
    objPanel.classList.remove("hidden");
    objPanel.classList.add("v112-context-open");
    return;
  }

  /* Резерв — стара панель властивостей інструмента V68 */
  const old=$112("v68ToolSettings");
  if(old){
    old.classList.remove("hidden");
    old.classList.add("v112-context-open");
    return;
  }

  contextEmpty112().innerHTML="<b>Контекстне меню</b><br>Для цього об’єкта немає окремих параметрів.";
  contextEmpty112().classList.add("show");
}

/* Не дозволяємо старим selection handlers самостійно показувати контекст.
   Користувач відкриває його лише кнопкою "Контекст". */
function suppressAutoContext112(){
  const ids=["textFormatBar","v102ObjectPanel","v68ToolSettings"];

  const repair=()=>{
    if($112("v112ContextBtn")?.classList.contains("active")) return;

    ids.forEach(id=>{
      const p=$112(id);
      if(!p)return;
      p.classList.remove("v112-context-open");
      if(id==="textFormatBar") p.classList.add("hidden");
    });
  };

  const c=c112();
  if(c && !c.__v112SelectionBound){
    c.__v112SelectionBound=true;
    ["selection:created","selection:updated","selection:cleared","text:editing:entered"].forEach(ev=>{
      c.on(ev,()=>setTimeout(repair,0));
    });
  }
}

/* Після зміни активного об'єкта, якщо Контекст уже відкритий —
   оновлюємо його під новий об'єкт. */
function liveContext112(){
  const c=c112();
  if(!c || c.__v112LiveContext)return;
  c.__v112LiveContext=true;

  ["selection:created","selection:updated"].forEach(ev=>{
    c.on(ev,()=>{
      if(!$112("v112ContextBtn")?.classList.contains("active")) return;
      setTimeout(()=>{
        closeContext112();
        $112("v112ContextBtn")?.classList.add("active");
        openContext112();
      },0);
    });
  });
...
function resizeCanvas112(){
  const c=c112(), nb=$112("notebook");
  if(!c || !nb) return;

  const rect=nb.getBoundingClientRect();
  const collapsed=document.body.classList.contains("v112-right-collapsed");
  const reserve=collapsed?0:74;
  const w=Math.max(600,window.innerWidth-rect.left-reserve);
...
function repair112(){
  css112();
  ensureDockVisible112();
  ensureToggle112();
  ensureContextButton112();
  contextEmpty112();
  suppressAutoContext112();
  liveContext112();
}

function init112(){
  repair112();
  restoreRightState112();
  mark112();
  resizeCanvas112();
...
window.addEventListener("resize",()=>setTimeout(resizeCanvas112,100));

if(document.readyState==="loading"){
  document.addEventListener("DOMContentLoaded",()=>setTimeout(init112,220),{once:true});
}else{
  setTimeout(init112,220);
}
})();
...
/* =========================================================
   V113 — МЕНЮ НЕ ПЕРЕКРИВАЄ ПРАВУ ПАНЕЛЬ + ГРАФІК ПРАЦЮЄ
   ========================================================= */
(function(){
"use strict";
...
function css113(){
  ...
  st.id="v113Css";
  st.textContent=`
    /* Головне вікно команд не заходить під праву панель */
    #v86Commands{
      position:fixed!important;
      left:12px!important;
      right:86px!important;
    ...
    /* Самі вкладки всередині займають доступну ширину */
    ...
    /* Великі вкладені панелі: графік, математика, медіа тощо */
    #graphBuilderPanel,
    #graphEditorPanel,
    #numberRayPanel,
    #geometryPanel,
    ...
    #aiPanel{
      ...
      box-sizing:border-box!important;
      z-index:130100!important;
    }

    /* Поля у побудові графіка не вилазять за межі */
    #graphBuilderPanel input,
    #graphBuilderPanel select,
    #graphBuilderPanel button,
    #graphBuilderPanel label,
    #graphBuilderPanel .graph-param{
      max-width:100%!important;
      box-sizing:border-box!important;
    }

    /* Права панель завжди окремо справа */
    #v86Dock{
      z-index:129000!important;
    }

    /* На вузьких екранах лишаємо 72 px під праву панель */
    @media(max-width:900px){
      #v86Commands{
        left:6px!important;
...
/* Підняти відкрите меню на передній план */
function front113(){
  const p=$113("v86Commands");
  if(p?.classList.contains("show")){
    p.style.setProperty("z-index","130000","important");
  }
}

/* ---------- 2. ГРАФІК: НАДІЙНА ВСТАВКА ---------- */
function buildGraph113(){
  try{
    const c=c113();
    if(!c) throw new Error("Canvas не готовий");

    const type=$113("graphType")?.value || "linear";
    const name=$113("graphName")?.value?.trim() || "Графік";

    // Координатна площина
    if($113("paperType")){
      $113("paperType").value="coordinate";
    ...
    const meta={
      name,
      type,
      params:getParamObjectFromPanel($113("graphParams")),
      customExpr:$113("customGraphExpression")?.value?.trim() || "",
      showPoints:!!$113("graphShowPoints")?.checked,
      shiftX:0,
      shiftY:0,
      range:readGraphRange(),
      autoScale:!!$113("graphAutoScale")?.checked,
      clipToPlane:!!$113("graphClipToPlane")?.checked,
      color:$113("colorPicker")?.value || "#17315f",
      strokeWidth:Math.max(2,Number($113("lineWidth")?.value)||2)
    };

    const g=createGraphGroup(meta);

    // КРИТИЧНО: не дозволяємо старому курсору V97 пересунути графік.
    ...
    // Графік має лежати точно на координатній площині.
    g.set({
      left:0,
      top:0,
      originX:"left",
    ...
    try{
      graphCounter++;
      if($113("graphName"))$113("graphName").value=`Графік ${graphCounter}`;
    }catch(_){}

    try{pushHistory();autoSave()}catch(_){}

    // Закриваємо конструктор тільки ПІСЛЯ успішної побудови.
    $113("graphBuilderPanel")?.classList.add("hidden");

    try{openGraphEditor(g)}catch(_){}

    return g;
  }catch(err){
    console.error("V113 graph error:",err);
    alert("Не вдалося побудувати графік: "+(err?.message||err));
    return null;
  }
}

function bindGraph113(){
  const b=$113("insertGraphBtn");
  if(!b || b.__v113Graph)return;
  b.__v113Graph=true;

  // Повністю замінюємо стару дію кнопки, щоб функція не запускалась двічі.
  b.onclick=null;
  b.addEventListener("click",e=>{
    e.preventDefault();
    e.stopPropagation();
...
/* Усі нові/оновлені графіки захищаємо від пересування курсором */
function protectGraphs113(){
  const c=c113();
  if(!c || c.__v113GraphProtect)return;
  c.__v113GraphProtect=true;
...
function init113(){
  repair113();
  mark113();
  [300,900,1600].forEach(ms=>setTimeout(repair113,ms));
}

document.addEventListener("click",e=>{
  if(e.target.closest?.("#v86Dock,.v86tab,#graphBuilderBtn")){
    setTimeout(front113,30);
  }
},true);

if(document.readyState==="loading")
  document.addEventListener("DOMContentLoaded",()=>setTimeout(init113,220),{once:true});
else
