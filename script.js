...
/* =========================================================
   V120 — stable V107 base + bottom page-bar finishing only
   ========================================================= */
(function(){
"use strict";
const byId=id=>document.getElementById(id);
...
function addCss(){
 if(byId("v120Css")) return;
 const st=document.createElement("style");
 st.id="v120Css";
 st.textContent=`
   /* Keep page navigation at the bottom without touching canvas/workspace geometry */
   #v99PageDock{
     position:fixed!important;
     left:88px!important;
     right:78px!important;
...
function repairPageBar(){
 const add=byId("addPageBtn");
 if(add){
   add.hidden=false;
   add.textContent="+ Нова сторінка";
   add.title="Створити нову сторінку";
   add.setAttribute("aria-label","Нова сторінка");
 }

 // Remove only duplicate Parasochka signatures.
 const label="Sofia Notebook © Parasochka";
 const sigs=[...document.querySelectorAll("body *")]
   .filter(el=>(el.textContent||"").trim()===label);
 let keep=byId("sofiaAuthorSignature") || sigs[0] || null;
 if(keep){
   keep.id="sofiaAuthorSignature";
   keep.textContent=label;
   sigs.forEach(el=>{ if(el!==keep) el.remove(); });
 }
}

function mark(){
 const b=byId("appVersionBadge") ||
   [...document.querySelectorAll("span,small,b")].find(x=>/^v\d+$/i.test((x.textContent||"").trim()));
 if(b)b.textContent="v120";
 document.documentElement.dataset.sofiaVersion="120";
}
...
function init(){
 ...
 repairPageBar();
 mark();
 [500,1200].forEach(ms=>setTimeout(repairPageBar,ms));
}
if(document.readyState==="loading")
